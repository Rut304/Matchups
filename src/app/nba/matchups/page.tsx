...
import { Calendar, RefreshCw, Loader2, Clock, ChevronRight, Filter, TrendingUp } from 'lucide-react'
...
export default function NBAMatchupsPage() {
  ...
  const fetchGames = async () => {
    try {
      setLoading(true)
      const res = await fetch('/api/games?sport=nba')
      if (res.ok) {
        ...
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-black text-white">NBA Matchups</h1>
            ...
            {liveGames.length > 0 && (
              <div>
                ...
                <div className="grid gap-3">
                  {liveGames.map(game => (
                    <GameCard key={game.id} game={game} formatTime={formatTime} />
                  ))}
                </div>
              </div>
            )}
...
function GameCard({ game, formatTime }: { game: Game; formatTime: (d: string) => string }) {
  ...
  return (
    <Link href={`/nba/matchups/${game.id}`}>
      <div className={`p-4 rounded-xl bg-[#0c0c14] border ${isLive ? 'border-green-500/50' : 'border-white/5'} hover:border-orange-500/30 transition-all cursor-pointer`}>
        <div className="flex items-center justify-between">
          ...
          {/* Right Side - Odds & Status */}
          <div className="flex items-center gap-6">
            {game.odds && (
              <div className="flex gap-4 text-sm">
                <div className="text-center">
                  <div className="text-xs text-gray-500">SPREAD</div>
                  <div className="text-orange-400 font-bold">{game.odds.spread > 0 ? '+' : ''}{game.odds.spread}</div>
                </div>
                <div className="text-center">
                  <div className="text-xs text-gray-500">O/U</div>
                  <div className="text-green-400 font-bold">{game.odds.total}</div>
                </div>
              </div>
            )}
