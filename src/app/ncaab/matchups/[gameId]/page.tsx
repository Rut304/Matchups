  ...
  if (error || !game) {
    return (
      <ErrorDisplay
        ...
        message="This NCAAB game couldn't be found. It may have been rescheduled or cancelled."
        backLink="/ncaab/matchups"
        backText="Back to NCAAB matchups"
        showRetry={false}
      />
    )
  }
    ...
    >
      <MatchupLayout.Grid>
        <MatchupLayout.MainContent>
          ...
          {trends && trends.matched > 0 && trends.spreadTrends && (
            <div className="bg-[#0c0c14] rounded-xl border border-white/10 p-4">
              ...
              <Link href="/trends?sport=ncaab" className="text-xs text-orange-400 hover:underline mt-2 inline-block">View all →</Link>
            </div>
          )}
        ...
        <MatchupLayout.Sidebar>
          ...
          <GameInfo game={game} />
          <div className="space-y-2">
            <Link href="/trends?sport=ncaab" className="flex items-center justify-between p-3 bg-[#0c0c14] rounded-xl border border-white/10 hover:border-orange-500/30 transition-all group">
              <div className="flex items-center gap-2"><TrendingUp className="w-4 h-4 text-orange-500" /><span className="text-sm text-white group-hover:text-orange-400">All NCAAB Trends</span></div>
