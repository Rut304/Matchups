...
export default function NCAABMatchupDetailPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [gamesRes, analyticsRes] = await Promise.all([fetch('/api/games?sport=ncaab'), fetch(`/api/matchup/${params.gameId}/analytics?intelligence=true`)])
  ...
  if (!game) return <div className="min-h-screen bg-[#050508] flex flex-col items-center justify-center gap-4"><div className="text-gray-500">Game not found</div><Link href="/ncaab/matchups" className="text-orange-400 hover:underline">← Back to matchups</Link></div>
  ...
  return (
    <div className="min-h-screen bg-[#050508]">
      <div className="max-w-6xl mx-auto px-4 py-6">
        <Link href="/ncaab/matchups" className="inline-flex items-center gap-2 text-gray-400 hover:text-white mb-6"><ArrowLeft className="w-4 h-4" /> Back to NCAAB Matchups</Link>
