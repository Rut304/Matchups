...
export default function NCAABMatchupsPage() {
  ...
  const fetchGames = async () => {
    try { setLoading(true); const res = await fetch('/api/games?sport=ncaab'); if (res.ok) { const data = await res.json(); setGames(data.games || []); setLastUpdated(new Date()) } } catch (e) { console.error(e) } finally { setLoading(false) }
  }
      ...
      <div className="border-b border-white/5 bg-[#0a0a12]">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3"><span className="text-3xl">🏀</span><div><h1 className="text-2xl font-black text-white">College Basketball Matchups</h1><p className="text-sm text-gray-500">NCAAB odds, trends & analysis</p></div></div>
...
function GameCard({ game, formatTime }: { game: Game; formatTime: (d: string) => string }) {
  ...
  return (
    <Link href={`/ncaab/matchups/${game.id}`}>
