...
export default function WNCAABGameMatchupPage({ params }: { params: Promise<{ gameId: string }> }) {
  ...
  if (error || !game) {
    return (
      <ErrorDisplay
        ...
        message="This WNCAAB game couldn't be found. It may have been rescheduled or cancelled."
        backLink="/wncaab/matchups"
        backText="Back to WNCAAB matchups"
        showRetry={false}
      />
    )
  }
  ...
  // Key numbers for college basketball (3, 6, 7, 10)
    ...
    >
      <MatchupLayout.Grid>
        <MatchupLayout.MainContent>
          {/* WNCAAB: Key Number Alert */}
          {isKeyNumber && (
            <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-3">
              <div className="flex items-center gap-2">
                <Trophy className="w-4 h-4 text-blue-400" />
                <span className="text-sm text-blue-400 font-medium">Key Number: {Math.abs(Math.round(spread))}</span>
                <span className="text-xs text-gray-500">— Common margin in WCBB</span>
              </div>
            </div>
          )}
          ...
          {trends && trends.matched > 0 && trends.spreadTrends && (
            <div className="bg-[#0c0c14] rounded-xl border border-white/10 p-4">
              ...
              <Link href="/trends?sport=wncaab" className="text-xs text-orange-400 hover:underline mt-2 inline-block">View all →</Link>
            </div>
          )}
        ...
        <MatchupLayout.Sidebar>
          ...
          <GameInfo game={game} />
          <div className="space-y-2">
            <Link href="/trends?sport=wncaab" className="flex items-center justify-between p-3 bg-[#0c0c14] rounded-xl border border-white/10 hover:border-orange-500/30 transition-all group">
              <div className="flex items-center gap-2"><TrendingUp className="w-4 h-4 text-orange-500" /><span className="text-sm text-white group-hover:text-orange-400">All WNCAAB Trends</span></div>
