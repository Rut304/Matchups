...
export default function WNCAABMatchupsPage() {
  ...
  const fetchGames = async () => {
    try { setLoading(true); const res = await fetch('/api/games?sport=wncaab'); if (res.ok) { const data = await res.json(); setGames(data.games || []); setLastUpdated(new Date()) } } catch (e) { console.error(e) } finally { setLoading(false) }
  }
  ...
  return (
    <div className="min-h-screen bg-[#050508]">
      <div className="border-b border-white/5 bg-[#0a0a12]">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3"><span className="text-3xl">🏀</span><div><h1 className="text-2xl font-black text-white">Women's College Basketball Matchups</h1><p className="text-sm text-gray-500">WNCAAB odds, trends & analysis</p></div></div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        {loading && games.length === 0 ? <div className="flex items-center justify-center py-20"><Loader2 className="w-8 h-8 text-orange-500 animate-spin" /></div> : filteredGames.length === 0 ? <div className="text-center py-20 text-gray-500">No games scheduled for this period</div> : (
...
function GameCard({ game, formatTime }: { game: Game; formatTime: (d: string) => string }) {
  ...
  return (
    <Link href={`/wncaab/matchups/${game.id}`}>
