...
/**
 * NCAAF GAME MATCHUP — "The Whale" Edition
 * Sport-specific: Key numbers (3,7,10,14,17,21), weather (outdoor), home field advantage
 */
...
export default function NCAAFGameMatchupPage({ params }: { params: Promise<{ gameId: string }> }) {
  ...
  useEffect(() => {
    if (!game) return
    Promise.all([
      fetch(`/api/team/ncaaf/${game.homeTeam.abbreviation}/schedule?limit=15`).then(r => r.ok ? r.json() : null),
      fetch(`/api/team/ncaaf/${game.awayTeam.abbreviation}/schedule?limit=15`).then(r => r.ok ? r.json() : null),
    ]).then(([h, a]) => {
  ...
  if (error || !game) return <ErrorDisplay variant="full" title="Game Not Found" message="This NCAAF game could not be found." backLink="/ncaaf/matchups" backText="Back to NCAAF matchups" showRetry={false} />
        ...
        <MatchupLayout.MainContent>
          ...
          {isKeyNumber && (
            <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg px-3 py-2 flex items-center gap-2">
              ...
              <span className="text-xs text-gray-400">{Math.abs(spread)} — common CFB margin of victory</span>
            </div>
          )}
          ...
          {/* Power Ratings — Key for college */}
          <div className="relative">
            <span className="absolute top-2.5 right-2 z-10"><Tooltip content={TOOLTIPS.powerRating} /></span>
            <PowerRatingsComparison sport="ncaaf" homeTeam={game.homeTeam.abbreviation} awayTeam={game.awayTeam.abbreviation} />
          </div>
          ...
          {trends && trends.matched > 0 && (
            <CollapsibleSection title="Betting Trends" icon={TrendingUp} badge={trends.matched}>
              <div className="space-y-1.5 mt-2">
                ...
                <Link href="/trends?sport=ncaaf" className="text-[10px] text-orange-400 hover:underline">View all →</Link>
              </div>
            </CollapsibleSection>
          )}
          ...
          <CollapsibleSection title="Player Props" icon={Target}>
            <div className="mt-2"><GamePlayerProps gameId={gameId} sport="NCAAF" homeTeam={game.homeTeam.name} awayTeam={game.awayTeam.name} /></div>
          </CollapsibleSection>

        </MatchupLayout.MainContent>
        <MatchupLayout.Sidebar>
          {edgeScore && edgeScore.overall > 0 && <EdgeScoreCard edgeScore={edgeScore} gameId={gameId} />}
          {/* Weather — NCAAF is outdoor */}
          <div className="relative">
            <span className="absolute top-2.5 right-2 z-10"><Tooltip content={TOOLTIPS.weather} /></span>
            <WeatherPanel venue={game.venue || ''} city={game.venue?.split(',').pop()?.trim() || ''} gameDate={game.scheduledAt || game.startTime} sport="ncaaf" compact={false} />
          </div>
          ...
          <div className="space-y-1.5">
            <Link href="/trends?sport=ncaaf" className="flex items-center justify-between p-2.5 bg-[#0c0c14] rounded-lg border border-white/5 hover:border-orange-500/30 transition-all group text-xs">
              <div className="flex items-center gap-2"><TrendingUp className="w-3.5 h-3.5 text-orange-500" /><span className="text-gray-300 group-hover:text-orange-400">All NCAAF Trends</span></div>
