 ...
 * NCAAF-specific: Key numbers (3,7,10,14,17,21), weather (outdoor), home field, NO officials
 */
...
export default function NCAAFGameMatchupPage({ params }: { params: Promise<{ gameId: string }> }) {
  ...
  useEffect(() => {
    ...
    fetchIntelligence(gameId, 'NCAAF', game).then(data => {
    ...
    <MatchupLayout sport={sport} game={game} analytics={analytics} isLoading={isLoading} lastUpdated={lastUpdated} onRefresh={handleRefresh} activeTab={activeTab} onTabChange={setActiveTab}>
      <MatchupLayout.Grid>
        <MatchupLayout.MainContent>
          ...
          {/* Key Number — uses Math.round via shared component */}
