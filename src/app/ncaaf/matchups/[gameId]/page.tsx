...
export default function NCAAFGameMatchupPage({ params }: { params: Promise<{ gameId: string }> }) {
  ...
  useEffect(() => {
    if (!game) return
    Promise.all([
      fetch(`/api/team/ncaaf/${game.homeTeam.abbreviation}/schedule?limit=5`).then(r => r.ok ? r.json() : null),
      fetch(`/api/team/ncaaf/${game.awayTeam.abbreviation}/schedule?limit=5`).then(r => r.ok ? r.json() : null),
    ]).then(([h, a]) => {
