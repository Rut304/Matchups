...
export default function NCAAFMatchupsPage() {
  ...
  const fetchGames = async () => {
    try { setLoading(true); const res = await fetch('/api/games?sport=ncaaf'); if (res.ok) { const data = await res.json(); setGames(data.games || []); setLastUpdated(new Date()) } } catch (e) { console.error(e) } finally { setLoading(false) }
  }
  ...
  return (
    <div className="min-h-screen bg-[#050508]">
      <div className="border-b border-white/5 bg-[#0a0a12]">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3"><span className="text-3xl">🏈</span><div><h1 className="text-2xl font-black text-white">College Football Matchups</h1><p className="text-sm text-gray-500">NCAAF odds, trends & analysis</p></div></div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        {loading && games.length === 0 ? <div className="flex items-center justify-center py-20"><Loader2 className="w-8 h-8 text-orange-500 animate-spin" /></div> : filteredGames.length === 0 ? <div className="text-center py-20 text-gray-500">No games scheduled - College football bowl season is over</div> : (
...
function GameCard({ game, formatTime }: { game: Game; formatTime: (d: string) => string }) {
  ...
  return (
    <Link href={`/ncaaf/matchups/${game.id}`}>
        ...
        <div className="flex items-center justify-between">
          <div className="flex-1">
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-3 min-w-[200px]">{game.awayTeam.logo && <img src={game.awayTeam.logo} alt="" className="w-10 h-10 object-contain" />}<div><div className="font-bold text-white">{game.awayTeam.name}</div><div className="text-xs text-gray-500">{game.awayTeam.record || ''}</div></div>{(isLive || isFinal) && <div className="text-2xl font-black text-white ml-auto">{game.awayTeam.score}</div>}</div>
              <span className="text-gray-600 font-bold">@</span>
              <div className="flex items-center gap-3 min-w-[200px]">{game.homeTeam.logo && <img src={game.homeTeam.logo} alt="" className="w-10 h-10 object-contain" />}<div><div className="font-bold text-white">{game.homeTeam.name}</div><div className="text-xs text-gray-500">{game.homeTeam.record || ''}</div></div>{(isLive || isFinal) && <div className="text-2xl font-black text-white ml-auto">{game.homeTeam.score}</div>}</div>
            </div>
          </div>
          <div className="flex items-center gap-6">
            {game.odds && <div className="flex gap-4 text-sm"><div className="text-center"><div className="text-xs text-gray-500">SPREAD</div><div className="text-orange-400 font-bold">{game.odds.spread > 0 ? '+' : ''}{game.odds.spread}</div></div><div className="text-center"><div className="text-xs text-gray-500">O/U</div><div className="text-green-400 font-bold">{game.odds.total}</div></div></div>}
            <div className="text-right min-w-[80px]">{isLive ? <div className="flex items-center gap-1.5 text-green-400 text-sm font-bold"><span className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse" />{game.period} {game.clock}</div> : isFinal ? <div className="text-gray-500 text-sm font-medium">FINAL</div> : <div className="text-gray-400 text-sm">{formatTime(game.scheduledAt)}</div>}{game.broadcast && <div className="text-xs text-gray-600">{game.broadcast}</div>}</div>
