 ...
 * Sport-specific: B2B alert, goalie matchup, Puck Line labels, Officials, NO weather (indoor)
 */
...
import { 
  ...
  AlertTriangle, Calendar, Moon, Target, Shield
} from 'lucide-react'
...
export default function NHLGameMatchupPage({ params }: { params: Promise<{ gameId: string }> }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`/api/team/nhl/${game.homeTeam.abbreviation}/schedule?limit=5`).then(r => r.ok ? r.json() : null),
      fetch(`/api/team/nhl/${game.awayTeam.abbreviation}/schedule?limit=5`).then(r => r.ok ? r.json() : null),
    ]).then(([h, a]) => {
      ...
      <MatchupLayout.Grid>
        <MatchupLayout.MainContent>
          ...
          {(homeCtx?.isBackToBack || awayCtx?.isBackToBack) && (
            <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg px-3 py-2 flex items-center gap-2 flex-wrap">
              ...
              <span className="text-xs font-bold text-amber-400">B2B ALERT</span>
              {homeCtx?.isBackToBack && <span className="text-xs bg-amber-500/20 px-2 py-0.5 rounded text-amber-300"><Moon className="w-3 h-3 inline mr-1" />{game.homeTeam.abbreviation}</span>}
              {awayCtx?.isBackToBack && <span className="text-xs bg-amber-500/20 px-2 py-0.5 rounded text-amber-300"><Moon className="w-3 h-3 inline mr-1" />{game.awayTeam.abbreviation}</span>}
              <span className="text-[10px] text-gray-500">NHL B2B: ~48% PL cover rate</span>
            </div>
          )}
          ...
          {trends && trends.matched > 0 && (
            <CollapsibleSection title="Betting Trends" icon={TrendingUp} badge={trends.matched}>
              <div className="space-y-1.5 mt-2">
                ...
                <Link href="/trends?sport=nhl" className="text-[10px] text-orange-400 hover:underline">View all →</Link>
              </div>
            </CollapsibleSection>
          )}
          ...
          <CollapsibleSection title="Player Props" icon={Target}>
            <div className="mt-2"><GamePlayerProps gameId={gameId} sport="NHL" homeTeam={game.homeTeam.name} awayTeam={game.awayTeam.name} /></div>
          </CollapsibleSection>
        ...
        <MatchupLayout.Sidebar>
          ...
          <div className="space-y-1.5">
            <Link href="/trends?sport=nhl" className="flex items-center justify-between p-2.5 bg-[#0c0c14] rounded-lg border border-white/5 hover:border-orange-500/30 transition-all group text-xs">
              <div className="flex items-center gap-2"><TrendingUp className="w-3.5 h-3.5 text-orange-500" /><span className="text-gray-300 group-hover:text-orange-400">All NHL Trends</span></div>
